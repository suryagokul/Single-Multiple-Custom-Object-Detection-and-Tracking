...
while True:
    ...
    for track in tracker.tracks:
        ...
        cv2.line(img, (0, int(3*height/6-height/20)), (width, int(3*height/6-height/20)), (0, 0, 255), thickness=2) #entrance

        center_y = int(((bbox[1])+(bbox[3]))/2)
        
        CoorYEntranceLine = int(3*height/6-height/20)
        
        CoorYExitLine     = int(3*height/6+height/20)

        #print(CoorYEntranceLine,CoorYExitLine) 486, 594

        #print("Centroid : ",center_y)
        ...
        if center_y <= int(3*height/6+height/20) and center_y >= int(3*height/6-height/20):
            if class_name == 'person':
                ...
                if (CheckEntranceLineCrossing(center_y,CoorYEntranceLine,CoorYExitLine)):
                  EntranceCounter += 1
                
                if (CheckExitLineCrossing(center_y,CoorYEntranceLine,CoorYExitLine)):
                  ExitCounter += 1

    total_count = len(set(counter))
    cv2.putText(img, "Current Persons Count: " + str(current_count), (0, 80), 0, 1, (0, 0, 255), 2)
    cv2.putText(img, "Total Persons Count: " + str(total_count), (0,130), 0, 1, (0,0,255), 2)
    cv2.putText(img, "Entrances: {}".format(str(EntranceCounter)), (0, 180),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    cv2.putText(img, "Exits: {}".format(str(ExitCounter)), (0, 230),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
